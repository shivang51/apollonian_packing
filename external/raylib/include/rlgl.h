/**********************************************************************************************
 *
 *   rlgl v5.0 - A multi-OpenGL abstraction layer with an immediate-mode style
 *API
 *
 *   DESCRIPTION:
...
#define RLAPI                                                                  \
    __attribute__((visibility("default"))) // We are building the library as a
...
typedef struct rlVertexBuffer {
    ...
    float *vertices;       // Vertex position (XYZ - 3 components per vertex)
                           // (shader-location = 0)
    float *texcoords;      // Vertex texture coordinates (UV - 2 components per
...
#endif
#if defined(GRAPHICS_API_OPENGL_ES2)
    unsigned short *indices; // Vertex indices (in case vertex data comes
                             // indexed) (6 indices per quad)
#endif
...
typedef struct rlDrawCall {
    ...
    int vertexAlignment; // Number of vertex required for index alignment
                         // (LINES, TRIANGLES)
    ...
    unsigned int textureId; // Texture id to be used on the draw -> Use to
                            // create new draw call if changes
...
typedef enum {
    ...
    RL_BLEND_ALPHA_PREMULTIPLY, // Blend premultiplied textures considering
                                // alpha
...
typedef enum {
    ...
    RL_SHADER_LOC_COLOR_DIFFUSE,     // Shader location: vector uniform: diffuse
                                     // color
    RL_SHADER_LOC_COLOR_SPECULAR, // Shader location: vector uniform: specular
                                  // color
    RL_SHADER_LOC_COLOR_AMBIENT,  // Shader location: vector uniform: ambient
                                  // color
    RL_SHADER_LOC_MAP_ALBEDO,     // Shader location: sampler2d texture: albedo
                                  // (same as: RL_SHADER_LOC_MAP_DIFFUSE)
    RL_SHADER_LOC_MAP_METALNESS,  // Shader location: sampler2d texture:
                                  // metalness (same as:
                                  // RL_SHADER_LOC_MAP_SPECULAR)
    RL_SHADER_LOC_MAP_NORMAL,     // Shader location: sampler2d texture: normal
    RL_SHADER_LOC_MAP_ROUGHNESS,  // Shader location: sampler2d texture:
                                  // roughness
    RL_SHADER_LOC_MAP_OCCLUSION,  // Shader location: sampler2d texture:
                                  // occlusion
...
// NOTE: This functions are used to completely abstract raylib code from OpenGL
// layer, some of them are direct wrappers over OpenGL calls, some others are
// custom
//------------------------------------------------------------------------------------
...
RLAPI void
rlDisableVertexArray(void); // Disable vertex array (VAO, if supported)
...
#if defined(GRAPHICS_API_OPENGL_11)
RLAPI void rlEnableStatePointer(int vertexAttribType,
...
RLAPI void
rlTextureParameters(unsigned int id, int param,
                    int value); // Set texture parameters (filter, wrap)
RLAPI void
rlCubemapParameters(unsigned int id, int param,
...
RLAPI unsigned int
rlGetActiveFramebuffer(void); // Get the currently active render texture (fbo),
                              // 0 for default framebuffer
RLAPI void
...
RLAPI void
rlBindFramebuffer(unsigned int target,
                  unsigned int framebuffer); // Bind framebuffer (FBO)
...
RLAPI void
rlDisableWireMode(void); // Disable wire mode ( and point ) maybe rename
...
RLAPI void
rlClearScreenBuffers(void);     // Clear used screen buffers (color and depth)
RLAPI void rlCheckErrors(void); // Check and log OpenGL error codes
RLAPI void rlSetBlendMode(int mode); // Set blending mode
RLAPI void rlSetBlendFactors(int glSrcFactor, int glDstFactor,
...
RLAPI void rlSetBlendFactorsSeparate(
    ...
    int glEqAlpha); // Set blending mode factors and equations separately (using
                    // OpenGL factors)
...
// immediate mode but this render batch API is exposed in case of custom batches
// are required
...
RLAPI void rlSetRenderBatchActive(
    rlRenderBatch *batch); // Set the active render batch for rlgl (NULL for
                           // default internal)
RLAPI void
rlDrawRenderBatchActive(void); // Update and draw internal render batch
RLAPI bool rlCheckRenderBatchLimit(
    int vCount); // Check internal buffer overflow for a given number of vertex
...
RLAPI unsigned int
rlLoadVertexArray(void); // Load vertex array (vao) if supported
RLAPI unsigned int
rlLoadVertexBuffer(const void *buffer, int size,
                   bool dynamic); // Load a vertex buffer object
RLAPI unsigned int
rlLoadVertexBufferElement(const void *buffer, int size,
                          bool dynamic); // Load vertex buffer elements object
RLAPI void rlUpdateVertexBuffer(
    unsigned int bufferId, const void *data, int dataSize,
    int offset); // Update vertex buffer object data on GPU buffer
RLAPI void rlUpdateVertexBufferElements(
    unsigned int id, const void *data, int dataSize,
    int offset); // Update vertex buffer elements data on GPU buffer
RLAPI void rlUnloadVertexArray(unsigned int vaoId); // Unload vertex array (vao)
RLAPI void
rlUnloadVertexBuffer(unsigned int vboId); // Unload vertex buffer object
RLAPI void rlSetVertexAttribute(
    unsigned int index, int compSize, int type, bool normalized, int stride,
    const void *pointer); // Set vertex attribute data configuration
RLAPI void
rlSetVertexAttributeDivisor(unsigned int index,
                            int divisor); // Set vertex attribute data divisor
RLAPI void
rlSetVertexAttributeDefault(int locIndex, const void *value, int attribType,
                            int count); // Set vertex attribute default value,
                                        // when attribute to provided
RLAPI void
rlDrawVertexArray(int offset,
                  int count); // Draw vertex array (currently active vao)
RLAPI void
rlDrawVertexArrayElements(int offset, int count,
                          const void *buffer); // Draw vertex array elements
RLAPI void rlDrawVertexArrayInstanced(
    int offset, int count,
    int instances); // Draw vertex array (currently active vao) with instancing
RLAPI void rlDrawVertexArrayElementsInstanced(
    int offset, int count, const void *buffer,
    int instances); // Draw vertex array elements with instancing

// Textures management
RLAPI unsigned int rlLoadTexture(const void *data, int width, int height,
                                 int format,
                                 int mipmapCount); // Load texture data
RLAPI unsigned int
rlLoadTextureDepth(int width, int height,
                   bool useRenderBuffer); // Load depth texture/renderbuffer (to
                                          // be attached to fbo)
RLAPI unsigned int
rlLoadTextureCubemap(const void *data, int size,
                     int format); // Load texture cubemap data
RLAPI void
rlUpdateTexture(unsigned int id, int offsetX, int offsetY, int width,
                int height, int format,
                const void *data); // Update texture with new data on GPU
RLAPI void
rlGetGlTextureFormats(int format, unsigned int *glInternalFormat,
...
RLAPI void
rlGenTextureMipmaps(unsigned int id, int width, int height, int format,
...
RLAPI bool
rlFramebufferComplete(unsigned int id); // Verify framebuffer is complete
RLAPI void rlUnloadFramebuffer(unsigned int id); // Delete framebuffer from GPU

// Shaders management
RLAPI unsigned int
rlLoadShaderCode(const char *vsCode,
...
RLAPI unsigned int
rlLoadShaderProgram(unsigned int vShaderId,
                    unsigned int fShaderId);       // Load custom shader program
RLAPI void rlUnloadShaderProgram(unsigned int id); // Unload shader program
RLAPI int
rlGetLocationUniform(unsigned int shaderId,
                     const char *uniformName); // Get shader location uniform
RLAPI int
rlGetLocationAttrib(unsigned int shaderId,
...
RLAPI void
rlSetUniformSampler(int locIndex,
                    unsigned int textureId); // Set shader value sampler
RLAPI void
rlSetShader(unsigned int id,
...
RLAPI unsigned int
rlLoadShaderBuffer(unsigned int size, const void *data,
...
RLAPI void rlUpdateShaderBuffer(unsigned int id, const void *data,
...
RLAPI void
rlReadShaderBuffer(unsigned int id, void *dest, unsigned int count,
                   unsigned int offset); // Read SSBO buffer data (GPU->CPU)
RLAPI void
rlCopyShaderBuffer(unsigned int destId, unsigned int srcId,
                   unsigned int destOffset, unsigned int srcOffset,
                   unsigned int count); // Copy SSBO data between buffers
RLAPI unsigned int
rlGetShaderBufferSize(unsigned int id); // Get SSBO buffer size
...
RLAPI Matrix
rlGetMatrixTransform(void); // Get internal accumulated transform matrix
RLAPI Matrix rlGetMatrixProjectionStereo(
    int eye); // Get internal projection matrix for stereo render (selected eye)
RLAPI Matrix
rlGetMatrixViewOffsetStereo(int eye); // Get internal view offset matrix for
                                      // stereo render (selected eye)
RLAPI void
rlSetMatrixProjection(Matrix proj); // Set a custom projection matrix (replaces
                                    // internal projection matrix)
RLAPI void
rlSetMatrixModelview(Matrix view); // Set a custom modelview matrix (replaces
...
typedef struct rlglData {
    ...
    struct {
        int vertexCounter; // Current active render batch vertex counter
                           // (generic, used for all batches)
        float texcoordx, texcoordy; // Current active texture coordinate (added
                                    // on glVertex*())
        ...
        Matrix transform;       // Transform matrix to be used with rlTranslate,
                                // rlRotate, rlScale
        bool transformRequired; // Require transform matrix application to
                                // current draw-call vertex (if required)
        ...
        unsigned int activeTextureId
            [RL_DEFAULT_BATCH_MAX_TEXTURE_UNITS]; // Active texture ids to be
                                                  ...
                                                  // active by default)
        unsigned int defaultVShaderId; // Default vertex shader id (used by
                                       // default shader program)
        unsigned int defaultFShaderId; // Default fragment shader id (used by
                                       // default shader program)
        unsigned int defaultShaderId;  // Default shader program id, supports
                                       // vertex color and diffuse texture
        int *defaultShaderLocs; // Default shader locations pointer to be used
                                // on rendering
        unsigned int currentShaderId; // Current shader id to be used on
                                      // rendering (by default, defaultShaderId)
        int *currentShaderLocs; // Current shader locations pointer to be used
                                // on rendering (by default, defaultShaderLocs)

        bool stereoRender; // Stereo rendering flag
        Matrix
            projectionStereo[2]; // VR stereo rendering eyes projection matrices
        Matrix viewOffsetStereo[2]; // VR stereo rendering eyes view offset
                                    // matrices
...
void rlMultMatrixf(const float *matf) {
    ...
    *RLGL.State.currentMatrix =
        rlMatrixMultiply(*RLGL.State.currentMatrix, mat);
}
...
void rlBegin(int mode) {
    ...
    if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1].mode !=
        ...
        if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
                .vertexCount > 0) {
            // Make sure current RLGL.currentBatch->draws[i].vertexCount is
            // aligned a multiple of 4, that way, following QUADS drawing will
            // keep aligned with index processing It implies adding some extra
            // alignment vertex at the end of the draw, those vertex are not
            // processed but they are considered as an additional offset for the
...
void rlEnd(void) {
    ...
    // Correct increment formula would be: depthInc = (zfar - znear)/pow(2,
    // bits)
...
void rlVertex3f(float x, float y, float z) {
    ...
    // RL_QUADS come in groups of 4 vertices. We must check current draw.mode
    // when a new vertex is required and finish the batch only if the draw.mode
    ...
    if (RLGL.State.vertexCounter >
        ...
        if ((RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
            ...
            // Launch a draw call but keep current state for next vertices
            // comming NOTE: We add +1 vertex to the check for security
    ...
    if (id == 0) {
        ...
        if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
            ...
            if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
                    .vertexCount > 0) {
                // Make sure current RLGL.currentBatch->draws[i].vertexCount is
                // aligned a multiple of 4, that way, following QUADS drawing
                // will keep aligned with index processing It implies adding
                // some extra alignment vertex at the end of the draw, those
                // vertex are not processed but they are considered as an
                // additional offset for the next set of vertex to be drawn
                if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
                        .mode == RL_LINES)
                    RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
                        .vertexAlignment =
                        ((RLGL.currentBatch
                             ...
                             ? RLGL.currentBatch
                                   ...
                                   .vertexCount
                             : RLGL.currentBatch
                                       ->draws[RLGL.currentBatch->drawCounter -
                                               1]
                                       .vertexCount %
                                   4);
                else if (RLGL.currentBatch
    ...
    case RL_TEXTURE_WRAP_T: {
        if (value == RL_TEXTURE_WRAP_MIRROR_CLAMP) {
            ...
            if (RLGL.ExtSupported.texMirrorClamp)
                ...
                TRACELOG(RL_LOG_WARNING, "GL: Clamp mirror wrap mode not "
                                         "supported (GL_MIRROR_CLAMP_EXT)");
#endif
        } else
            glTexParameteri(GL_TEXTURE_2D, param, value);
    ...
    case RL_TEXTURE_FILTER_ANISOTROPIC: {
        ...
        if (value <= RLGL.ExtSupported.maxAnisotropyLevel)
            ...
            TRACELOG(RL_LOG_WARNING,
                     ...
                     id, (int)RLGL.ExtSupported.maxAnisotropyLevel);
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                            (float)value);
        } else
...
#if ((defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES3)) &&  \
     defined(RLGL_RENDER_TEXTURES_HINT))
    // NOTE: Maximum number of draw buffers supported is implementation
    // dependant, it can be queried with glGet*() but it must be at least 8
...
void rlClearScreenBuffers(void) {
    glClear(GL_COLOR_BUFFER_BIT |
            GL_DEPTH_BUFFER_BIT); // Clear used buffers: Color and Depth (Depth
                                  // is used for 3D)
    // glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT |
    // GL_STENCIL_BUFFER_BIT);     // Stencil buffer not used...
}
...
void rlSetBlendMode(int mode) {
    ...
    if ((RLGL.State.currentBlendMode != mode) ||
        ...
        case RL_BLEND_CUSTOM: {
            // NOTE: Using GL blend src/dst factors and GL equation configured
            // with rlSetBlendFactors()
            glBlendFunc(RLGL.State.glBlendSrcFactor,
                        RLGL.State.glBlendDstFactor);
            glBlendEquation(RLGL.State.glBlendEquation);

        } break;
        case RL_BLEND_CUSTOM_SEPARATE: {
            // NOTE: Using GL blend src/dst factors and GL equation configured
            // with rlSetBlendFactorsSeparate()
            glBlendFuncSeparate(RLGL.State.glBlendSrcFactorRGB,
...
void rlglInit(int width, int height) {
    ...
    if ((glDebugMessageCallback != NULL) && (glDebugMessageControl != NULL)) {
        ...
        //  breakpoint can be placed on the callback in order to get a
        //  stacktrace for the GL error
    ...
    glBlendFunc(GL_SRC_ALPHA,
                GL_ONE_MINUS_SRC_ALPHA); // Color blending function (how colors
                                         // are mixed)
    glEnable(GL_BLEND); // Enable color blending (required to work with
                        // transparencies)
...
#if defined(GRAPHICS_API_OPENGL_33)
    glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS); // Seamless cubemaps (not supported
                                            // on OpenGL ES 2.0)
#endif
...
void rlLoadExtensions(void *loader) {
    ...
    for (int i = 0; i < numExt; i++) {
        // Check VAO support
        // NOTE: Only check on OpenGL ES, OpenGL 3.3 has VAO support as core
        // feature
        if (strcmp(extList[i], (const char *)"GL_OES_vertex_array_object") ==
            0) {
            ...
            glGenVertexArrays = (PFNGLGENVERTEXARRAYSOESPROC)((
                rlglLoadProc)loader)("glGenVertexArraysOES");
            glBindVertexArray = (PFNGLBINDVERTEXARRAYOESPROC)((
                rlglLoadProc)loader)("glBindVertexArrayOES");
            glDeleteVertexArrays = (PFNGLDELETEVERTEXARRAYSOESPROC)((
                rlglLoadProc)loader)("glDeleteVertexArraysOES");
            // glIsVertexArray =
            // (PFNGLISVERTEXARRAYOESPROC)loader("glIsVertexArrayOES");     //
            // NOTE: Fails in WebGL, omitted
            ...
            0) // Web ANGLE
        {
            ...
            if ((strcmp(extList[i], (const char *)"GL_EXT_draw_instanced") ==
                 0) && // Standard EXT
                (strcmp(extList[i], (const char *)"GL_EXT_instanced_arrays") ==
                 0)) {
                glDrawArraysInstanced = (PFNGLDRAWARRAYSINSTANCEDEXTPROC)((
        ...
        // NOTE: Only check on OpenGL ES, OpenGL 3.3 has NPOT textures full
        ...
        if ((strcmp(extList[i],
                    (const char *)"GL_EXT_texture_compression_s3tc") == 0) ||
            (strcmp(extList[i],
                    (const char *)"GL_WEBGL_compressed_texture_s3tc") == 0) ||
            (strcmp(extList[i],
                    (const char *)"GL_WEBKIT_WEBGL_compressed_texture_s3tc") ==
             0))
            RLGL.ExtSupported.texCompDXT = true;

        // Check texture compression support: ETC1
        if ((strcmp(extList[i],
                    (const char *)"GL_OES_compressed_ETC1_RGB8_texture") ==
             0) ||
            (strcmp(extList[i],
                    (const char *)"GL_WEBGL_compressed_texture_etc1") == 0))
            RLGL.ExtSupported.texCompETC1 = true;
        ...
        if (strcmp(extList[i],
                   (const char *)"GL_IMG_texture_compression_pvrtc") == 0)
            RLGL.ExtSupported.texCompPVRT = true;
        ...
        if (strcmp(extList[i],
                   (const char *)"GL_EXT_texture_filter_anisotropic") == 0)
            RLGL.ExtSupported.texAnisoFilter = true;

        // Check clamp mirror wrap mode support
        if (strcmp(extList[i], (const char *)"GL_EXT_texture_mirror_clamp") ==
            0)
            RLGL.ExtSupported.texMirrorClamp = true;
    }
...
#define GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT 0x84FF
    ...
    if (RLGL.ExtSupported.texNPOT)
        TRACELOG(RL_LOG_INFO, "GL: NPOT textures extension detected, full NPOT "
                              "textures supported");
    else
        TRACELOG(RL_LOG_WARNING,
                 "GL: NPOT textures extension not found, limited NPOT support "
                 "(no-mipmaps, no-repeat)");
...
rlRenderBatch rlLoadRenderBatch(int numBuffers, int bufferElements) {
    ...
    for (int i = 0; i < numBuffers; i++) {
        ...
        batch.vertexBuffer[i].indices = (unsigned int *)RL_MALLOC(
            bufferElements * 6 *
            sizeof(unsigned int)); // 6 int by quad (indices)
#endif
#if defined(GRAPHICS_API_OPENGL_ES2)
        batch.vertexBuffer[i].indices = (unsigned short *)RL_MALLOC(
            bufferElements * 6 *
            sizeof(unsigned short)); // 6 int by quad (indices)
#endif
    ...
    //--------------------------------------------------------------------------------------------
    batch.draws = (rlDrawCall *)RL_MALLOC(RL_DEFAULT_BATCH_DRAWCALLS *
                                          sizeof(rlDrawCall));
...
void rlDrawRenderBatch(rlRenderBatch *batch) {
    ...
    if (RLGL.State.vertexCounter > 0) {
        ...
        // batch->vertexBuffer[batch->currentBuffer].texcoords,
        // GL_DYNAMIC_DRAW); // Update all buffer
        ...
        // first glBufferData() with NULL pointer before glMapBuffer(). If you
        // do that, the previous data in PBO will be discarded and glMapBuffer()
        // returns a new allocated pointer immediately even if GPU is still
    ...
    for (int eye = 0; eye < eyeCount; eye++) {
        if (eyeCount == 2) {
            ...
            rlViewport(eye * RLGL.State.framebufferWidth / 2, 0,
                       RLGL.State.framebufferWidth / 2,
                       RLGL.State.framebufferHeight);

            // Set current eye view offset to modelview matrix
            rlSetMatrixModelview(rlMatrixMultiply(
                matModelView, RLGL.State.viewOffsetStereo[eye]));
        ...
        if (RLGL.State.vertexCounter > 0) {
            ...
            glUniformMatrix4fv(
                RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_MVP], 1,
                false, matMVPfloat);

            if (RLGL.ExtSupported.vao)
                glBindVertexArray(
                    batch->vertexBuffer[batch->currentBuffer].vaoId);
            else {
                ...
                glEnableVertexAttribArray(
                    RLGL.State
                        .currentShaderLocs[RL_SHADER_LOC_VERTEX_POSITION]);
                ...
                glVertexAttribPointer(
                    RLGL.State
                        .currentShaderLocs[RL_SHADER_LOC_VERTEX_TEXCOORD01],
                    2, GL_FLOAT, 0, 0, 0);
                glEnableVertexAttribArray(
                    RLGL.State
                        .currentShaderLocs[RL_SHADER_LOC_VERTEX_TEXCOORD01]);
            ...
            glUniform4f(
                RLGL.State.currentShaderLocs[RL_SHADER_LOC_COLOR_DIFFUSE], 1.0f,
                1.0f, 1.0f, 1.0f);
            glUniform1i(RLGL.State.currentShaderLocs[RL_SHADER_LOC_MAP_DIFFUSE],
                        0); // Active default sampler2D: texture0

            // Activate additional sampler textures
            // Those additional textures will be common for all draw calls of
            // the batch
            ...
            // Activate default sampler2D texture0 (one texture is always active
            // for default batch shader) NOTE: Batch system accumulates calls by
            // texture0 changes, additional textures are enabled for all the
            // draw calls
            glActiveTexture(GL_TEXTURE0);

            for (int i = 0, vertexOffset = 0; i < batch->drawCounter; i++) {
                // Bind current draw call texture, activated as GL_TEXTURE0 and
                // Bound to sampler2D texture0 by default
                ...
                if ((batch->draws[i].mode == RL_LINES) ||
                    ...
                    // elementCount*6 NOTE: The final parameter tells the GPU
                    // the offset in bytes from the start of the index buffer to
                    // the location of the first index to process
                    glDrawElements(
                        GL_TRIANGLES, batch->draws[i].vertexCount / 4 * 6,
                        GL_UNSIGNED_INT,
                ...
                vertexOffset += (batch->draws[i].vertexCount +
                                 batch->draws[i].vertexAlignment);
            }
    ...
    if (eyeCount == 2)
        rlViewport(0, 0, RLGL.State.framebufferWidth,
                   RLGL.State.framebufferHeight);
    //------------------------------------------------------------------------------------------------------------
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format,
    ...
    for (int i = 0; i < mipmapCount; i++) {
        ...
        TRACELOGD(
            "TEXTURE: Load mipmap level %i (%i x %i), size: %i, offset: %i", i,
            mipWidth, mipHeight, mipSize, mipOffset);

        if (glInternalFormat != 0) {
            if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
                glTexImage2D(GL_TEXTURE_2D, i, glInternalFormat, mipWidth,
                             mipHeight, 0, glFormat, glType, dataPtr);
#if !defined(GRAPHICS_API_OPENGL_11)
            else
    ...
    // NOTE: glTexParameteri does NOT affect texture uploading, just the way
    // it's used
#if defined(GRAPHICS_API_OPENGL_ES2)
    ...
    if (id > 0)
        TRACELOG(RL_LOG_INFO,
                 "TEXTURE: [ID %i] Texture loaded successfully (%ix%i | %s | "
                 "%i mipmaps)",
...
unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer) {
    ...
    if (!useRenderBuffer && RLGL.ExtSupported.texDepth) {
        ...
        // Create the renderbuffer that will serve as the depth attachment for
        // the framebuffer NOTE: A renderbuffer is simpler than a texture and
        // could offer better performance on embedded devices
        ...
        TRACELOG(
            ...
            id,
            (RLGL.ExtSupported.maxDepthBits >= 24)
                ? RLGL.ExtSupported.maxDepthBits
                : 16);
    }
#endif
                ...
                if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) {
                    if ((format == RL_PIXELFORMAT_UNCOMPRESSED_R32) ||
                        ...
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
                                     glInternalFormat, size, size, 0, glFormat,
                                     glType, NULL);
                } else
                    TRACELOG(RL_LOG_WARNING,
                             "TEXTURES: Empty cubemap creation does not "
                             "support compressed format");
            } else {
                if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
                    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
                                 glInternalFormat, size, size, 0, glFormat,
                                 glType, (unsigned char *)data + i * dataSize);
                else
                    glCompressedTexImage2D(
                        GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glInternalFormat,
...
void rlUnloadTexture(unsigned int id) { glDeleteTextures(1, &id); }
...
#define MAX(a, b) (((a) > (b)) ? (a) : (b))
        ...
        *mipmaps = 1 + (int)floor(log(MAX(width, height)) / log(2));
        ...
        TRACELOG(RL_LOG_WARNING, "TEXTURE: [ID %i] Failed to generate mipmaps",
                 id);
...
void *rlReadTexturePixels(unsigned int id, int width, int height, int format) {
    ...
    if ((glInternalFormat != 0) &&
        ...
        TRACELOG(RL_LOG_WARNING,
                 "TEXTURE: [ID %i] Data retrieval not suported for pixel "
                 "format (%i)",
                 id, format);
    ...
    // Texture width and height are required on OpenGL ES 2.0. There is no way
    // to get it from texture id. Two possible Options: 1 - Bind texture to
    // color fbo attachment and glReadPixels() 2 - Create an fbo, activate it,
    // render quad with texture, glReadPixels() We are using Option 1, just need
    // to care for texture format on retrieval NOTE: This behaviour could be
    // conditioned by graphic driver...
...
unsigned char *rlReadScreenPixels(int width, int height) {
    ...
    // bottom left corner of the framebuffer NOTE 2: We are getting alpha
    // channel! Be careful, it can be transparent if not cleared properly!
    ...
    for (int y = height - 1; y >= 0; y--) {
        for (int x = 0; x < (width * 4); x++) {
            ...
            // NOTE: Alpha value has already been applied to RGB in framebuffer,
            // we don't need it!
            if (((x + 1) % 4) == 0)
    ...
    case RL_ATTACHMENT_COLOR_CHANNEL7: {
        if (texType == RL_ATTACHMENT_TEXTURE2D)
            ...
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + attachType,
                GL_TEXTURE_CUBE_MAP_POSITIVE_X + texType, texId, mipLevel);
...
bool rlFramebufferComplete(unsigned int id) {
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        ...
        case GL_FRAMEBUFFER_UNSUPPORTED:
            TRACELOG(RL_LOG_WARNING, "FBO: [ID %i] Framebuffer is unsupported",
                     id);
            break;
...
void rlUnloadFramebuffer(unsigned int id) {
    ...
    // *currently bound* framebuffer, the texture image is automatically
...
void rlUnloadVertexArray(unsigned int vaoId) {
    ...
    if (RLGL.ExtSupported.vao) {
        ...
        TRACELOG(RL_LOG_INFO,
                 "VAO: [ID %i] Unloaded vertex array data from VRAM (GPU)",
                 vaoId);
    }
#endif
}
...
unsigned int rlLoadShaderCode(const char *vsCode, const char *fsCode) {
    ...
    // In case no vertex shader was provided or compilation failed, we use
    ...
    if ((vertexShaderId == RLGL.State.defaultVShaderId) &&
        ...
        // One of or both shader are new, we need to compile a new shader
        // program
        id = rlLoadShaderProgram(vertexShaderId, fragmentShaderId);

        // We can detach and delete vertex/fragment shaders (if not default
        // ones) NOTE: We detach shader before deletion to make sure memory is
        // freed
        if (vertexShaderId != RLGL.State.defaultVShaderId) {
        ...
        if (id == 0) {
            ...
            TRACELOG(RL_LOG_WARNING, "SHADER: Failed to load custom shader "
                                     "code, using default shader");
            id = RLGL.State.defaultShaderId;
        }
        /*
        else
        {
            ...
            for (int i = 0; i < uniformCount; i++)
            {
                int namelen = -1;
                int num = -1;
                char name[256] = { 0 };     // Assume no variable names longer
        than 256 GLenum type = GL_ZERO;

                // Get the name of the uniforms
                glGetActiveUniform(id, i, sizeof(name) - 1, &namelen, &num,
        &type, name);

                name[namelen] = 0;
                TRACELOGD("SHADER: [ID %i] Active uniform (%s) set at location:
        %i", id, name, glGetUniformLocation(id, name));
            }
        }
        */
    }
...
unsigned int rlCompileShader(const char *shaderCode, int type) {
    ...
    if (success == GL_FALSE) {
        ...
        case GL_VERTEX_SHADER:
            TRACELOG(RL_LOG_WARNING,
                     "SHADER: [ID %i] Failed to compile vertex shader code",
                     shader);
            break;
        ...
        case GL_COMPUTE_SHADER:
            TRACELOG(RL_LOG_WARNING,
                     "SHADER: [ID %i] Failed to compile compute shader code",
                     shader);
            break;
#endif
        default:
        ...
        if (maxLength > 0) {
            ...
            TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Compile error: %s",
                     shader, log);
            RL_FREE(log);
        }
    } else {
        ...
        case GL_VERTEX_SHADER:
            TRACELOG(RL_LOG_INFO,
                     "SHADER: [ID %i] Vertex shader compiled successfully",
                     shader);
            break;
        case GL_FRAGMENT_SHADER:
            TRACELOG(RL_LOG_INFO,
                     "SHADER: [ID %i] Fragment shader compiled successfully",
                     shader);
            break;
            // case GL_GEOMETRY_SHADER:
#if defined(GRAPHICS_API_OPENGL_43)
        case GL_COMPUTE_SHADER:
            TRACELOG(RL_LOG_INFO,
                     "SHADER: [ID %i] Compute shader compiled successfully",
                     shader);
            break;
#endif
        default:
...
unsigned int rlLoadShaderProgram(unsigned int vShaderId,
    ...
    if (success == GL_FALSE) {
        ...
        if (maxLength > 0) {
            ...
            TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Link error: %s", program,
                     log);
            RL_FREE(log);
        }
        ...
        // Get the size of compiled shader program (not available on OpenGL
        // ES 2.0) NOTE: If GL_LINK_STATUS is GL_FALSE, program binary length is
        // zero.
        // GLint binarySize = 0;
...
int rlGetLocationUniform(unsigned int shaderId, const char *uniformName) {
    ...
    // TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] Shader uniform (%s) set at
    // location: %i", shaderId, uniformName, location);
#endif
    return location;
}
...
void rlSetVertexAttributeDefault(int locIndex, const void *value,
    ...
    case RL_SHADER_ATTRIB_VEC4:
        ...
        break;
    default:
        TRACELOG(RL_LOG_WARNING, "SHADER: Failed to set attrib default value, "
...
unsigned int rlLoadComputeShaderProgram(unsigned int shaderId) {
    ...
    if (success == GL_FALSE) {
        TRACELOG(RL_LOG_WARNING,
                 "SHADER: [ID %i] Failed to link compute shader program",
                 program);
...
void rlBindImageTexture(unsigned int id, unsigned int index, int format,
    ...
    glBindImageTexture(index, id, 0, 0, 0,
                       readonly ? GL_READ_ONLY : GL_READ_WRITE,
                       glInternalFormat);
#endif
}
...
static void rlLoadShaderDefault(void) {
    ...
    RLGL.State.defaultShaderId = rlLoadShaderProgram(
        RLGL.State.defaultVShaderId, RLGL.State.defaultFShaderId);

    if (RLGL.State.defaultShaderId > 0) {
        TRACELOG(RL_LOG_INFO,
                 "SHADER: [ID %i] Default shader loaded successfully",
        ...
        TRACELOG(RL_LOG_WARNING,
                 "SHADER: [ID %i] Failed to load default shader",
                 RLGL.State.defaultShaderId);
}
...
static void rlUnloadShaderDefault(void) {
    ...
    TRACELOG(RL_LOG_INFO,
             "SHADER: [ID %i] Default shader unloaded successfully",
             RLGL.State.defaultShaderId);
}
